...
Hướng dẫn này đưa bạn từ zero đến một môi trường local đang chạy đầy đủ — bốn service, database có dữ liệu demo, và admin panel sẵn sàng để khám phá.
...
7. [Generate gRPC types](#7-generate-grpc-types)
8. [Chạy tất cả service](#8-chạy-tất-cả-service)
9. [Xác minh và khám phá](#9-xác-minh-và-khám-phá)
10. [Khởi động lại nhanh (các ngày sau)](#10-khởi-động-lại-nhanh-các-ngày-sau)
...
```bash
git clone https://github.com/AIVIETNAM-AIO-HUYTRUONG/VizTeckStack.git
cd VizTeckStack
```
...
VizTeckStack dùng PostgreSQL 16 chạy trong Docker container:
...
### apps/svc-roadmap — tạo file `apps/svc-roadmap/.env`
...
> **Lưu ý:** Next.js apps dùng `.env.local` (không phải `.env`). NestJS apps dùng `.env`. Các giá trị mặc định trên hoạt động ngay với môi trường local — không cần chỉnh sửa.
...
Datasource "db": PostgreSQL database "vizteckstack", schema "public" at "localhost:5432"
...
> **Windows PowerShell:** Cú pháp `KEY=value command` không hoạt động trên PowerShell gốc. Dùng Git Bash (đi kèm với Git for Windows) hoặc WSL để chạy lệnh trên. Ngoài ra, bạn có thể tạo file `.env` trong `packages/db/` với nội dung `DATABASE_URL=postgresql://vizteck:vizteck@localhost:5432/vizteckstack` rồi chạy `pnpm --filter @vizteck/db db:push` mà không cần prefix.

---

## 7. Generate gRPC types

`apps/svc-roadmap` và `apps/api-gateway` giao tiếp với nhau qua gRPC. TypeScript types được generate từ file `roadmap.proto`:

```bash
cd packages/proto && node generate.js && cd ../..
```

> **Lưu ý:** Trên Windows Command Prompt hoặc PowerShell, dùng dấu `\` và chạy từng lệnh riêng:
> ```powershell
> cd packages\proto
> node generate.js
> cd ..\..
...
```
Proto generation complete → D:\...\packages\proto\generated
```

File `packages/proto/generated/roadmap.ts` sẽ được tạo hoặc cập nhật.

> **Turborepo cache:** Lệnh `pnpm proto:gen` từ root có thể bị cache bởi Turborepo và replay kết quả cũ. Nếu bạn sửa file `.proto`, luôn dùng `cd packages/proto && node generate.js` để force regenerate.

---

## 8. Chạy tất cả service
...
Turborepo khởi động cả bốn service theo thứ tự dependency (database package → proto → svc-roadmap → api-gateway → web + admin).

**Chờ đến khi thấy đủ 4 dòng này** (có thể mất 30–60 giây lần đầu):
...
> **Thứ tự xuất hiện** có thể khác nhau mỗi lần — không cần lo lắng miễn là cả 4 dòng đều xuất hiện.

Giữ terminal này mở. Tất cả service sẽ tự động reload khi bạn sửa code (hot reload).

---

## 9. Xác minh và khám phá

### 9a. Kiểm tra từng service
...
## 10. Khởi động lại nhanh (các ngày sau)
...
| Sửa file `roadmap.proto` | `cd packages/proto && node generate.js` |
...
### `node generate.js` lỗi "protoc: command not found"

`protoc` (Protocol Buffer compiler) chưa được cài.
...
# Hoặc tải bản binary từ GitHub releases
...
Thay `3000` bằng port thực tế bị lỗi (`3000`, `3001`, `3002`, hoặc `5001`).
...
- `packages/proto/generated/roadmap.ts` chưa tồn tại → chạy lại [Bước 7](#7-generate-grpc-types)
...
```powershell
$env:DATABASE_URL = "postgresql://vizteck:vizteck@localhost:5432/vizteckstack"
...
- **[Tổng quan kiến trúc](./architecture.md)** — hiểu bốn service kết nối với nhau như thế nào
