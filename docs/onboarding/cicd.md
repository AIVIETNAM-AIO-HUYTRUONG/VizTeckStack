...
5. Cài protoc (Protocol Buffers compiler) qua apt-get
6. cd packages/proto && node generate.js
   → generate TypeScript types từ roadmap.proto
7. pnpm lint
   → thực chất là TypeScript type check (tsc --noEmit), không phải style lint
8. pnpm test
   → chạy Vitest (admin) + Jest (api-gateway, svc-roadmap) — KHÔNG chạy E2E Playwright
9. pnpm build
...
- Nếu bạn thay đổi `.proto` mà quên generate: bước 6 sẽ generate lại, nhưng bước 7 có thể fail nếu types mới không khớp với code hiện tại
...
cài protoc + proto:gen                cài protoc + proto:gen
...
Playwright E2E tests (`apps/e2e`) cần tất cả 4 apps đang chạy cùng lúc:
- `apps/web` trên `:3001`
- `apps/admin` trên `:3002`  
- `apps/api-gateway` trên `:3000`
- `apps/svc-roadmap` trên `:5001`
